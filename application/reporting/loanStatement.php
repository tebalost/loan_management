...
class MYPDF extends TCPDF {
    ...
    public function Statement(){
        ...
        while ($schedule = mysqli_fetch_assoc($getSchedule)) {
            if($schedule['debit']=="0.00"){
                array_push($this->debit, "");
            }
            else{
                array_push($this->debit, $schedule['debit']);
            }
            if($schedule['credit']=="0.00"){
                array_push($this->credit, "");
            }
            else{
                array_push($this->credit,$schedule['credit'], 2, ".", ",");;
            }
    ...
    public function ColoredTable($header) {
        ...
        while($index < $count){
            ...
            $this->Cell($w[3], 6, $this->debit[$index], 'LRTB', 0, 'R', $fill);
            $this->Cell($w[4], 6, $this->credit[$index], 'LRTB', 0, 'R', $fill);
...
<section class="">
 <br/><br/><strong>'.$data['title'].'&nbsp;'.$data['fname'].'&nbsp;'.$data['lname'].'</strong><p>'.$address.'</p>
