<div class="row">
    <?php
    $getCompanyInfo = mysqli_query($link, "SELECT * FROM systemset");
    ...
    $id = $_GET['id'];
    $select = mysqli_query($link, "SELECT * FROM borrowers WHERE id = '$id'");
    ...
    $selectLoan = mysqli_query($link, "SELECT * FROM loan_info WHERE id = '$loanId'");
    while ($loan = mysqli_fetch_array($selectLoan)) {
        ...
        $loan_date=substr($loan['application_date'],0,10);
    ...
    $check = mysqli_query($link, "SELECT * FROM emp_permission WHERE tid = '" . $_SESSION['tid'] . "' AND module_name = 'Borrower Details'");
    $get_check = mysqli_fetch_array($check);
    $pupdate = $get_check['pupdate'];
    $pread = $get_check['pread'];

    $check = mysqli_query($link, "SELECT * FROM emp_permission WHERE tid = '" . $_SESSION['tid'] . "' AND module_name = 'Loans Approval'");
    $get_check = mysqli_fetch_array($check);
    ...
    $check = mysqli_query($link, "SELECT * FROM emp_permission WHERE tid = '" . $_SESSION['tid'] . "' AND module_name = 'Payment'");
    $get_check = mysqli_fetch_array($check);
    if(mysqli_num_rows($check)>0) {
        $reversePayment = $get_check['pcreate'];
    }else{
        $reversePayment = 0;
    }
    ...
    <section class="content-header"><h1><?php echo $title . " " . $fname . " " . $lname; ?> </h1>
                    ...
                    <div class="col-sm-4">
                        <ul class="list-unstyled">
                            ...
                            <li><b>Mobile:</b> <?php echo $phone . " " ?>
                                <?php if(isset($phone)) {  ?>
                                <div class="btn-group-horizontal">
                                    <?php echo ($pcreate == 1) ? '<a type="button" class="btn-xs bg-red" href="newmessage.php?id=' . $_SESSION['tid'] . '&&mid=' . base64_encode("406") . '&&FPayrpmobile='.$phone.'"><i class="fa fa-envelope"></i> Send SMS</a>' : ''; ?>
                                </div>
                                <?php } ?>
                            </li>
                ...
                <div class="row">
                    <div class="col-sm-8">
                        <div class="btn-group-horizontal">
                            ...
                            <!--Ad a query that get gets the overpayment here -->
                            <?php
                            $checkOverpayment = mysqli_fetch_assoc(mysqli_query($link,"Select * from payments where account='$account' and balance<0 and status='C'"));
                            $balance=$checkOverpayment['balance'];
                            $paymentId=$checkOverpayment['id'];
                            $_SESSION['account'] = $account;
                            $_SESSION['payment_id'] = $paymentId;

                            if(isset($balance)){
                            ?>
                            <a type="button" class="btn bg-orange" href="#<?php echo $paymentId; ?>&myModal<?php echo $paymentId; ?>" title="Reimburse"> <i
                                        class="fa fa-money text-white"
                                        style="font-size: large"
                                        data-target="#paymentId<?php echo $paymentId; ?>"
                                        data-toggle="modal"> Reimburse</i></a>
                            <?php } ?>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="pull-left">
                            <div class="input-group-btn">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    <li><a href="loanApplication.php?id=<?php echo $_GET['id'];?>&&loanId=<?php echo $_GET['loanId'] ?>&&contract=<?php echo $_GET['contract'] ?>">Loan Application</a></li>
        ...
        <div class="panel panel-success">
            ...
            <?php if (isset($_POST['savePayment'])) {
                ...
                if(($amount_paid_today !== $expectedInstalment) && ($amount_paid_today>$expectedInstalment)){
                    ...
                    if($total_due==$amount_paid_today){
                        ...
                        if ($totalPaid > 0) {
                         ...
                         interest_payment='$interest_due', fees_payment='$fees_due', penalty_payment='$penalty_due', open_indicator='C', payment_tx_id='$txID', total_due='0' where id='$rowId_Current'");
                            ...
                            $totalPaid = $amount_paid_today;
                            $balance=$expectedInstalment-$amount_paid_today;
                            ...
                            /*if($balance>$penalty_due)
                                $penalty_due=$penalty_due;
                            if($balance>$fees_due)
                                $fees_due=$fees_due;
                            if($balance>$interest_due)
                                $interest_due=$interest_due;*/
                            //echo "$balance $principal_due";
                            if($balance>0)
                                $principal_due=$principal_due-$balance;

                            $update = mysqli_query($link, "update pay_schedule set payment='$amount_paid_today', principal_payment='$principal_due', 
                         interest_payment='$interest_due', fees_payment='$fees_due', open_indicator='C', penalty_payment='$penalty_due', payment_tx_id='$txID', total_due='0' where id='$rowId_Current'");
                ...
                if (!$insert) {
                    ...
                    if ($accountBalanceOrigional <='0') {
                        $menu=base64_encode('405');
                        $URL = "viewborrowersloan.php?id=$borrower&&mid=$menu&&loanId=$loanId";
                ...
                if ((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
                    ...
                    if (($ext != "exe") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload")) {
                        ...
                        if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
                            ...
                            $insert = mysqli_query($link, "INSERT INTO collateral VALUES(0,'$id','$tid','$name','$type_of_collateral','$model','$make','$serial_number','$estimated_price','$filename','$cimage','$observation','$loanId')");
                            if (!$insert) {
            ...
            <div class="col-md-14">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        ...
                        $id = $_GET['id'];
                        $search = mysqli_query($link, "SELECT * FROM collateral WHERE idm = '$id' and loan='$loanId'");
                                            ...
                                            if(mysqli_num_rows($maxDateActive)>0 && $status==""){
                                                ...
                                                <tr>
                                                    ...
                                                    <td><b>
                                                            <?php
                                                            $today = date('Y-m-d');
                                                            function dateDifference($release_date, $today, $differenceFormat = '%m Months %d days')
                                                            {
                                                            ...
                                                            $ovedueDays = dateDifference($date_time, $today, $differenceFormat = '%m');

                                                            if($ovedueDays==1){
                                                                ?>
                                                                <b style="color: red"><?php echo dateDifference($date_time, $today, $differenceFormat = '%m Month %d days'); ?></b></h4>

                                                            <?php } else if($ovedueDays>1){?>
                                                                <b style="color: red"><?php echo dateDifference($date_time, $today, $differenceFormat = '%m Months %d days'); ?></b></h4>
                                                            <?php }

                                                            else{?>
                                                                <b style="color: green"><?php echo dateDifference($date_time, $today, $differenceFormat = '%d days'); ?></b></h4>
                                                            <?php }
                                                            ?>
                                                        </b>
                                                    </td>
                                        ...
                                        <div class="box box-info">
                                            <div class="box-body table-responsive no-padding">
                                                <table id="daily_collections"
                                                    ...
                                                    <?php
                                                    $totalPaid = 0;
                                                    $selectPayments = mysqli_query($link, "SELECT * FROM payments WHERE customer = '$borrower' and account='$account'");
                                                    while ($payment = mysqli_fetch_assoc($selectPayments)) {
                                                        ...
                                                        $schedulePayment = mysqli_fetch_assoc(mysqli_query($link,"select sum(principal_payment), sum(interest_payment), sum(fees_payment),sum(penalty_payment) from pay_schedule where payment_tx_id='$transactionID' and open_indicator='C' group by payment_tx_id"));
                                                        ?>
                                                        <tr>
                                                            <td class="bg-gray"></td>
                                                            <th width="10%"
                                                            ...
                                                            <td class="bg-gray text-right"><strong><?php echo number_format($schedulePayment['sum(penalty_payment)'], 2, ".", ","); ?></td>
                                                            <td class="bg-gray"></td>
                                                    ...
                                                    <?php while ($schedule = mysqli_fetch_assoc($getSchedule)) {
                                                        ...
                                                        <td class="text-right">
                                                            <?php
                                                            $penalty_due = round($schedule['penalty'], 2);
                                                            echo "<b style='color: red'>".number_format($penalty_due, 2, ".", ",")."</b>";
                                                            $penalty_total += $penalty_due;
                                                            ?>
                                                        </td>
                                                        <td class="text-right">
                                                            <?php
                                                        ...
                                                        <td class="text-right">
                                                            ...
                                                            if($pending_due_total>=0) {
                                                                echo $monthToDateDue;
                                                                mysqli_query($link,"update pay_schedule set total_due='$pending_due_total' where id='$schedule_id'");
                                                            }
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-right">
                                                            <b><?php echo number_format($penalty_total, 2, ".", ","); ?></b>
                                                        </td>
                                                        <td class="text-right">
                                            ...
                                            <div class="box-body">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-6">
                                                        <select class="form-control" name="paymentMethod"
                                                            ...
                                                            <option value="00">Other (Cash, EFT, Mobile Money)</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-6">
                                                        <?php
                                                        $tid = $_SESSION['tid'];
                                                        $sele = mysqli_query($link, "SELECT * from user WHERE id = '$tid'");
                                        ...
                                        <div class="row" style="margin-right:0.2%;margin-left:0.2%;margin-top: 1%;">
                                            <div class="col-sm-12 table-responsive">
                                                ...
                                                <table id="example1" class="table table-bordered table-striped">
                                                    ...
                                                    <?php
                                                    $tid = $_SESSION['tid'];
                                                    $select = mysqli_query($link, "SELECT * FROM payments WHERE customer = '$id' and account='$account'");
                                                    if (mysqli_num_rows($select) == 0) {
                                                        ...
                                                        while ($row = mysqli_fetch_array($select)) {
                                                            ...
                                                            $reversedPayment = mysqli_query($link, "SELECT * FROM payments WHERE id='$id' and status='R'");

                                                            $getin = mysqli_query($link, "SELECT username FROM user WHERE id = '$user'");
...
//$accte = $have['account'];
                                                            ...
                                                            $select1 = mysqli_query($link, "SELECT * FROM systemset");
                                                            if(mysqli_num_rows($reversedPayment)>0){
                                                            ?>
                                                            <tr class="danger">
                                                                ...
                                                                <td class="text-bold"><?php echo substr($pay_date, 0, 10); ?></td>
                                                                <td class="text-bold"><?php echo $payment_method; ?></td>
                                                                <td  class="text-bold" align="right"><?php echo number_format($amount_paid, 2, ".", ","); ?></td>
                                                                <td class="text-bold" align="right"><?php echo number_format($balance, 2, ".", ","); ?></td>
                                                                <td class="text-bold"><?php echo $row['reference']; ?></td>
                                                                <td class="text-bold"><?php echo $agent; ?></td>
                                                                <td class="text-bold"><?php
                                                                    if($reversePayment==1){
                                                                        ?>
                                                                        <span class="label label-warning"><?php echo "Pending Reversal"; ?></span>
                                                                    </a>&nbsp;
                                                                    <?php } ?>
                                                            ...
                                                            ?>
                                                                <tr class="">
                                                                    ...
                                                                    <td><?php
                                                                        if($reversePayment==1){
                                                                            ?>
                                                                            <a href="view_pmt.php?id=<?php echo $id; ?>" title="Revert Payment">
                                                                                <i class="fa fa-refresh"></i>
                                                                            </a>&nbsp;
                                                                        <?php } ?>
                                                                        <a href="#myModal <?php echo $id; ?>" title="Print Receipt"> <i
                                                                                    class="fa fa-print"
                                                                                    data-target="#myModal<?php echo $id; ?>"
                                                                                    data-toggle="modal"></i></a>
                                                                    </td>
                                                                ...
                                                                <div class="table-responsive">
                                                                    <table id="example2"
                                                                        ...
                                                                        $allFees = mysqli_query($link, "SELECT fee_name, sum(fee_amount) FROM loan_fees where loan in 
                                                                                                (SELECT id FROM loan_info where status not in ('DECLINED','Pending','Pending Disbursement') and id='$loanId') and fee_name !='Interest' group by fee_name");
                                ...
                                <div class="tab-pane" id="tab_guarantor">
                                    ...
                                    <?php
                                    $id = $_GET['id'];
                                    $search = mysqli_query($link, "SELECT * FROM loan_guarantors WHERE borrower = '$id' and loan_id='$loanId'");
                                    if (mysqli_num_rows($search) == 1) {
                                        $row = mysqli_fetch_array($search);
                                        $name = $row['name'];
                                ...
                                <div class="tab-pane" id="tab_documents">
                                    <?php $se = mysqli_query($link, "SELECT * FROM battachment WHERE get_id = '$borrower'");
                                            ...
                                            <div class="row" style="margin-right:0.2%;margin-left:0.2%;margin-top: 1%;">
                                                <div class="col-sm-12 table-responsive">
                                                    <table id="example1" class="table table-bordered table-striped">
                                                        ...
                                                        <?php
                                                        $id = $_GET['id'];
                                                        $select = mysqli_query($link, "SELECT * FROM comments WHERE customer='$borrower'and account='$account'");
                                                        if (mysqli_num_rows($select) == 0) {
                                                            echo "<div class='alert alert-info'>No data found yet!.....Check back later!!</div>";
                                                        } else {
                                                            while ($row = mysqli_fetch_array($select)) {
